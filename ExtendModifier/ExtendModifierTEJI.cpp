...
CExtendModifierTEJI::CExtendModifierTEJI(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_EXTENDMODIFIER_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CExtendModifierTEJI::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CExtendModifierTEJI, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_LBN_SELCHANGE(IDC_LIST1, &CExtendModifierTEJI::OnLbnSelchangeList1)
	ON_BN_CLICKED(IDOK, &CExtendModifierTEJI::OnBnClickedOk)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CExtendModifierTEJI::OnSelchangeCombo9)
	ON_BN_CLICKED(IDC_BUTTON1, &CExtendModifierTEJI::OnBnClickedButton1)
END_MESSAGE_MAP()


// CExtendModifierTEJI 消息处理程序

BOOL CExtendModifierTEJI::OnInitDialog()
{
...
void CExtendModifierTEJI::OnPaint()
{
	if (IsIconic())
	{
...
//显示。
HCURSOR CExtendModifierTEJI::OnQueryDragIcon()
{
...
void CExtendModifierTEJI::OnLbnSelchangeList1()
{
	page = 0;
	combo[8].SetCurSel(0);
...
void CExtendModifierTEJI::OnBnClickedOk()
{
	int per = list.GetCurSel();
	for (int i = page * 8; i < page * 8 + 8; i++) {
...
void CExtendModifierTEJI::OnModeChange(bool mode_, HANDLE hpro_)
{
	// TODO: 在此添加控件通知处理程序代码
	mode = mode_;
	if (hpro_ != 0)
...
void CExtendModifierTEJI::loadData(int per)
{
	if (mode == false) {
		ifstream file("Effect.e5", ios::binary);
		if (file) {
			file.seekg(per * 256 * 4, ios::beg);
			file.read(reinterpret_cast<char*>(&person_effect), 256 * 4);
			file.close();
		}
	}
	else {
...
void CExtendModifierTEJI::SaveData(int per)
{
	if (mode == false) {
		ofstream file("Effect.e5", ios::binary | ios::in);
		if (file) {
			file.seekp(per * 256 * 4, ios::beg);
			file.write(reinterpret_cast<char*>(&person_effect), 256 * 4);
			file.close();
		}
	}
	else {
		char* addr;
		ReadProcessMemory(hpro, (LPVOID)(0x4F0004), &addr, 4, 0);
		WriteProcessMemory(hpro, (LPVOID)(addr + per * 256 * 4), &person_effect, 256 * 4, 0);
	}
}


void CExtendModifierTEJI::OnSelchangeCombo9()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CExtendModifierTEJI::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
	int per = list.GetCurSel();
	int list[256], cnt = 0;
